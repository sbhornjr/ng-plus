...
import { likeList, unlikeList } from '@/lib/queries/list'
...
export default function LikeButton({ listId, initialCount, initialLiked, userId }: {
    ...
    async function handleLike() {
        ...
        if (liked) {
            await unlikeList(supabase, listId, userId)
            setLiked(false)
            setCount(c => c - 1)
        } else {
            await likeList(supabase, listId, userId)
            setLiked(true)
            setCount(c => c + 1)
        }
    ...
    return (
        <button
            onClick={handleLike}
            disabled={loading}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-[3px] border
