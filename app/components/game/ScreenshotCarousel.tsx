    ...
    return (
        <div className="flex h-auto gap-4 py-4 items-center">
            <button onClick={() => setCurrentIndex(currentIndex > 0 ? currentIndex - 1 : screenshots.length - 1)} 
                className="shrink-0 px-2 w-8 h-8 rounded-[3px] overflow-hidden border border-(--color-accent) flex items-center justify-center bg-(--color-surface) text-(--color-accent) text-lg font-semibold">
                ←
            </button>
            <div className="shrink-0 rounded-[3px] overflow-hidden border border-(--color-border) shadow-2xl aspect-video relative w-9/10 h-full">
